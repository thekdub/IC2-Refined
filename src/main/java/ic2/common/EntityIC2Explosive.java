...
public class EntityIC2Explosive extends Entity {
  ...
  public EntityIC2Explosive(World world, double d, double d1, double d2, int i, float f, float f1, float f2,
                            Block block, DamageSource damagesource) {
    this(world);
    this.setPosition(d, d1, d2);
    float f3 = (float) (Math.random() * 3.141592653589793D * 2.0D);
    this.motX = -MathHelper.sin(f3 * 3.1415927F / 180.0F) * 0.02F;
    this.motY = 0.20000000298023224D;
    this.motZ = -MathHelper.cos(f3 * 3.1415927F / 180.0F) * 0.02F;
    this.lastX = d;
    this.lastY = d1;
    this.lastZ = d2;
    this.fuse = i;
  ...
  private void explode() {
    ExplosionIC2 explosionic2 =
        new ExplosionIC2(this.world, null, this.locX, this.locY, this.locZ, this.explosivePower, this.dropRate,
            this.damageVsEntitys, this.damageSource, this.igniter);
