...
public class TileEntityTesla extends TileEntityBlock implements IEnergySink {
  ...
  public boolean shock(int i) {
    List list = this.world.a(EntityLiving.class, AxisAlignedBB
        .a(this.x - 4, this.y - 4, this.z - 4, this.x + 5,
            this.y + 5, this.z + 5));
    ...
    for (int j = 0; j < list.size(); ++j) {
      ...
      for (int k = 0; k < i; ++k) {
        this.world.a("reddust", entityliving.locX + (double) this.world.random.nextFloat(),
            entityliving.locY + (double) (this.world.random.nextFloat() * 2.0F),
            entityliving.locZ + (double) this.world.random.nextFloat(), 0.0D, 0.0D, 1.0D);
      }
    }
  ...
  public boolean redstoned() {
    return this.world.isBlockIndirectlyPowered(this.x, this.y, this.z) ||
        this.world.isBlockIndirectlyPowered(this.x, this.y, this.z);
  }
