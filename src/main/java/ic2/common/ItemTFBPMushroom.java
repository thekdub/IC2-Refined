  ...
  public boolean terraform(World world, int i, int j, int k) {
    ...
    if (l == -1) {
      ...
      if (event.isCancelled()) {
        ...
        return this.growBlockWithDependancy(world, i, l, j, net.minecraft.server.Block.BIG_MUSHROOM_1.id,
            net.minecraft.server.Block.BROWN_MUSHROOM.id);
      }
    }
  }
  ...
  public boolean growBlockWithDependancy(World world, int i, int j, int k, int l, int i1) {
      ...
      for (k2 = k - 1; k2 < k + 1; ++k2) {
        for (j3 = j + 5; j3 > j - 2; --j3) {
          ...
          if (i1 == net.minecraft.server.Block.MYCEL.id) {
            if (l3 == i1 || l3 == net.minecraft.server.Block.BIG_MUSHROOM_1.id ||
                l3 == net.minecraft.server.Block.BIG_MUSHROOM_2.id) {
              break;
            }
          ...
          if (i1 == net.minecraft.server.Block.BROWN_MUSHROOM.id) {
            if (l3 == net.minecraft.server.Block.BROWN_MUSHROOM.id ||
                l3 == net.minecraft.server.Block.RED_MUSHROOM.id) {
              break;
            }
    
            if (l3 != 0 && this.growBlockWithDependancy(world, j1, j3, k2, net.minecraft.server.Block.BROWN_MUSHROOM.id,
                net.minecraft.server.Block.MYCEL.id)) {
              return true;
            }
          }
        }
    ...
    if (l == net.minecraft.server.Block.BROWN_MUSHROOM.id) {
      ...
      if (l == net.minecraft.server.Block.BIG_MUSHROOM_1.id) {
        ...
        if (((BlockMushroom) net.minecraft.server.Block.byId[j1])
            .grow(world, i, j + 1, k, world.random, false, null, null)) {
