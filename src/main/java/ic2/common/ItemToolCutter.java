      ...
      if (tileentitycable.tryRemoveInsulation()) {
        if (Platform.isSimulating()) {
          ...
          EntityItem entityitem =
              new EntityItem(world, (double) i + d, (double) j + d1, (double) k + d2, Ic2Items.rubber.cloneItemStack());
        ...
        if (Platform.isRendering()) {
          AudioManager.playOnce(new AudioPosition(world, (float) i + 0.5F, (float) j + 0.5F, (float) k + 0.5F),
              PositionSpec.Center, "Tools/InsulationCutters.ogg", true, AudioManager.defaultVolume);
        }
      }
    }
