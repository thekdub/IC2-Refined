...
public class TileEntityBarrel extends TileEntity {
  ...
  public void q_() {
    if (!this.isEmpty() && this.treetapSide < 2) {
      ...
      if (this.type == 1 && this.timeRatio < 5) {
        ...
        if ((double) this.age >= 24000.0D * Math.pow(3.0D, i)) {
          this.age = 0;
          ++this.timeRatio;
        }
      }
  ...
  public void enforceUndetailed() {
    if (this.detailed) {
      ...
      if (this.type == 1) {
        ...
        float f1 =
            this.boozeAmount <= 0 ? 10.0F : (float) (this.hopsCount + this.wheatCount) / (float) this.boozeAmount;
        if (f <= 0.25F) {
          this.hopsRatio = 0;
        }
  ...
  public int timeNedForRum(int i) {
    return (int) ((double) (1200 * i) * Math.pow(0.95D, i - 1));
  }
}
