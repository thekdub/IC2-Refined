    ...
    for (int j = 0; j < i; ++j) {
      ...
      if (obj != null) {
        if (obj instanceof Class) {
          if (itemstack.id < Block.byId.length && Block.byId[itemstack.id] != null &&
              ((Class) obj).isAssignableFrom(Block.byId[itemstack.id].getClass())) {
            return true;
          }
  
          if ((itemstack.id >= Block.byId.length || Block.byId[itemstack.id] == null) &&
              Item.byId[itemstack.id] != null && ((Class) obj).isAssignableFrom(Item.byId[itemstack.id].getClass())) {
            return true;
          }
        }
