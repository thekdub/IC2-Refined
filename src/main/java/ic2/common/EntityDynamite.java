...
public class EntityDynamite extends Entity {
  ...
  public EntityDynamite(World world, EntityLiving entityliving) {
    ...
    this.height = 0.0F;
    this.motX =
        -MathHelper.sin(this.yaw / 180.0F * 3.1415927F) * MathHelper.cos(this.pitch / 180.0F * 3.1415927F);
    this.motZ =
        MathHelper.cos(this.yaw / 180.0F * 3.1415927F) * MathHelper.cos(this.pitch / 180.0F * 3.1415927F);
  ...
  public void setDynamiteHeading(double d, double d1, double d2, float f, float f1) {
    ...
    this.lastPitch = this.pitch = (float) (Math.atan2(d1, f3) * 180.0D / 3.141592653589793D);
  ...
  public void setVelocity(double d, double d1, double d2) {
    ...
    if (this.lastPitch == 0.0F && this.lastYaw == 0.0F) {
      ...
      this.lastYaw = this.yaw = (float) (Math.atan2(d, d2) * 180.0D / 3.141592653589793D);
      this.lastPitch = this.pitch = (float) (Math.atan2(d1, f) * 180.0D / 3.141592653589793D);
  ...
  public void F_() {
    ...
    if (movingobjectposition != null) {
      ...
      f8 = MathHelper.sqrt(f4 * f4 + f6 * f6 + f5 * f5);
      ...
      this.locY += f6;
      this.locZ += f5;
      this.motX *= 0.75F - this.random.nextFloat();
      this.motY *= -0.30000001192092896D;
      this.motZ *= 0.75F - this.random.nextFloat();
      this.inGround = true;
    }
    else {
      this.locX += this.motX;
    ...
    for (this.pitch = (float) (Math.atan2(this.motY, f2) * 180.0D / 3.141592653589793D);
         this.pitch - this.lastPitch < -180.0F; this.lastPitch -= 360.0F) {
    }
    ...
    if (this.aU()) {
      ...
      for (int i = 0; i < 4; ++i) {
        f8 = 0.25F;
        this.world.a("bubble", this.locX - this.motX * (double) f8, this.locY - this.motY * (double) f8,
            this.locZ - this.motZ * (double) f8, this.motX, this.motY, this.motZ);
      }
  ...
  public void explode() {
    PointExplosion pointexplosion =
        new PointExplosion(this.world, this.owner, (int) this.locX, (int) this.locY, (int) this.locZ, 1.0F, 1.0F, 0.8F);
