...
public class WorldGenRubTree extends WorldGenerator {
  ...
  public boolean grow(World world, int i, int j, int k, Random random) {
    if (world != null && Ic2Items.rubberWood != null) {
          ...
          if (j1 < 4 || j1 < 7 && k1 > 1 || k1 > 2) {
            for (int i2 = i - 2; i2 <= i + 2; ++i2) {
              for (int j2 = k - 2; j2 <= k + 2; ++j2) {
                ...
                boolean flag = i2 > i - 2 && i2 < i + 2 && j2 > k - 2 && j2 < k + 2 ||
                    i2 > i - 2 && i2 < i + 2 && random.nextInt(k2) == 0 ||
                    j2 > k - 2 && j2 < k + 2 && random.nextInt(k2) == 0;
      ...
      System.out.println("[ERROR] Had a null that shouldn't have been. RubberTree did not spawn! w=" + world + " r=" +
          Ic2Items.rubberWood);
      return false;
    }
  }
  ...
  public int getGrowHeight(World world, int i, int j, int k) {
    if ((world.getTypeId(i, j - 1, k) == Block.GRASS.id || world.getTypeId(i, j - 1, k) == Block.DIRT.id) &&
        (world.getTypeId(i, j, k) == 0 || world.getTypeId(i, j, k) == Ic2Items.rubberSapling.id)) {
      int l;
