...
public class TileEntityMiner extends TileEntityElecMachine implements IHasGui, ISidedInventory {
  ...
  public void q_() {
    ...
    if (this.isOperating()) {
      ...
      if (this.inventory[3] != null && (Item.byId[this.inventory[3].id] instanceof ItemElectricToolDrill ||
          Item.byId[this.inventory[3].id] instanceof ItemElectricToolDDrill)) {
  ...
  public boolean mine() {
    if (this.targetY < 0) {
      ...
      if (!this.canReachTarget(this.targetX, this.targetY, this.targetZ, true)) {
      ...
      else if (this.canMine(this.world.getTypeId(this.targetX, this.targetY, this.targetZ),
          this.world.getData(this.targetX, this.targetY, this.targetZ))) {
        this.stuckOn = null;
        ++this.miningTicker;
        --this.energy;
        ...
        if ((j == net.minecraft.server.Block.WATER.id || j == net.minecraft.server.Block.STATIONARY_WATER.id ||
            j == net.minecraft.server.Block.LAVA.id || j == net.minecraft.server.Block.STATIONARY_LAVA.id) &&
            this.isAnyPumpConnected()) {
          return false;
        }
        else {
          this.miningTicker = -1;
  ...
  public void mineBlock() {
    ...
    if (i == net.minecraft.server.Block.WATER.id || i == net.minecraft.server.Block.STATIONARY_WATER.id ||
        i == net.minecraft.server.Block.LAVA.id || i == net.minecraft.server.Block.STATIONARY_LAVA.id) {
      flag = true;
      if (j != 0) {
        i = 0;
      }
    ...
    if (i != 0) {
      if (!flag) {
        ...
        StackUtil
            .distributeDrop(this, block.getBlockDropped(this.world, this.targetX, this.targetY, this.targetZ, j, 0));
      }
      else {
        if (i == net.minecraft.server.Block.WATER.id || i == net.minecraft.server.Block.STATIONARY_WATER.id) {
  ...
  public boolean withdrawPipe() {
    ...
    if (j != 0) {
      StackUtil.distributeDrop(this, net.minecraft.server.Block.byId[j]
          .getBlockDropped(this.world, this.x, i, this.z, this.world.getData(this.x, i, this.z), 0));
      this.world.setTypeId(this.x, i, this.z, 0);
    }
    
    if (this.inventory[2] != null && this.inventory[2].id != Ic2Items.miningPipe.id &&
        this.inventory[2].id < net.minecraft.server.Block.byId.length &&
        net.minecraft.server.Block.byId[this.inventory[2].id] != null) {
      ...
      if (j > 0) {
        for (int k = this.x - j; k <= this.x + j; ++k) {
          for (int l = this.z - j; l <= this.z + j; ++l) {
            ...
            if (ItemScanner.isValuable(i1, j1) && this.canMine(i1, j1) ||
                this.isAnyPumpConnected() && this.world.getData(k, i, l) == 0 &&
                    (i1 == net.minecraft.server.Block.LAVA.id || i1 == net.minecraft.server.Block.STATIONARY_LAVA.id)) {
  ...
  public int getPipeTip() {
    int i;
    for (i = this.y; this.world.getTypeId(this.x, i - 1, this.z) == Ic2Items.miningPipe.id ||
        this.world.getTypeId(this.x, i - 1, this.z) == Ic2Items.miningPipeTip.id; --i) {
    }
  ...
  public boolean canPass(int i) {
    return i == 0 || i == net.minecraft.server.Block.WATER.id || i == net.minecraft.server.Block.STATIONARY_WATER.id ||
        i == net.minecraft.server.Block.LAVA.id || i == net.minecraft.server.Block.STATIONARY_LAVA.id ||
        i == Ic2Items.miner.id || i == Ic2Items.miningPipe.id || i == Ic2Items.miningPipeTip.id;
  }
  ...
  public boolean canMine(int i, int j) {
    ...
    else if (i != Ic2Items.miningPipe.id && i != Ic2Items.miningPipeTip.id &&
        i != net.minecraft.server.Block.CHEST.id) {
      if ((i == net.minecraft.server.Block.WATER.id || i == net.minecraft.server.Block.STATIONARY_WATER.id ||
          i == net.minecraft.server.Block.LAVA.id || i == net.minecraft.server.Block.STATIONARY_LAVA.id) &&
          this.isPumpConnected()) {
        return true;
      }
      else {
        net.minecraft.server.Block block = net.minecraft.server.Block.byId[i];
        if (block.getHardness(j) < 0.0F) {
          ...
          return true;
            ...
            this.inventory[3].id == Ic2Items.miningDrill.id && this.inventory[3].id == Ic2Items.diamondDrill.id) {
          return false;
        }
        else {
          try {
  ...
  public boolean canWithdraw() {
    return this.world.getTypeId(this.x, this.y - 1, this.z) == Ic2Items.miningPipe.id ||
        this.world.getTypeId(this.x, this.y - 1, this.z) == Ic2Items.miningPipeTip.id;
  }
  
  public boolean isPumpConnected() {
    if (this.world.getTileEntity(this.x, this.y + 1, this.z) instanceof TileEntityPump &&
        ((TileEntityPump) this.world.getTileEntity(this.x, this.y + 1, this.z)).canHarvest()) {
      return true;
    }
      ...
      return true;
        ...
        ((TileEntityPump) this.world.getTileEntity(this.x + 1, this.y, this.z)).canHarvest()) {
      return true;
    }
    else if (this.world.getTileEntity(this.x - 1, this.y, this.z) instanceof TileEntityPump &&
        ((TileEntityPump) this.world.getTileEntity(this.x - 1, this.y, this.z)).canHarvest()) {
    ...
    else if (this.world.getTileEntity(this.x, this.y, this.z + 1) instanceof TileEntityPump &&
        ((TileEntityPump) this.world.getTileEntity(this.x, this.y, this.z + 1)).canHarvest()) {
      return true;
    }
    else {
      return this.world.getTileEntity(this.x, this.y, this.z - 1) instanceof TileEntityPump &&
          ((TileEntityPump) this.world.getTileEntity(this.x, this.y, this.z - 1)).canHarvest();
    }
  }
  ...
  public boolean isAnyPumpConnected() {
    if (this.world.getTileEntity(this.x, this.y + 1, this.z) instanceof TileEntityPump) {
      ...
      return this.world.getTileEntity(this.x, this.y, this.z + 1) instanceof TileEntityPump ||
          this.world.getTileEntity(this.x, this.y, this.z - 1) instanceof TileEntityPump;
    }
  }
    ...
    if (s.equals("active") && this.prevActive != this.getActive()) {
      if (this.audioSource == null) {
        this.audioSource = AudioManager
            .createSource(this, PositionSpec.Center, "Machines/MinerOp.ogg", true, false, AudioManager.defaultVolume);
      }
