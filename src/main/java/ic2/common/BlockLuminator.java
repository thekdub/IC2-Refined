...
public class BlockLuminator extends BlockContainerCommon implements ITextureProvider {
  ...
  public static float[] getBoxOfLuminator(IBlockAccess iblockaccess, int i, int j, int k) {
    ...
    switch (l) {
      ...
      default:
        return flag ? new float[]{0.0F, 15.0F * f, 0.0F, 1.0F, 1.0F, 1.0F} :
            new float[]{4.0F * f, 13.0F * f, 4.0F * f, 1.0F - 4.0F * f, 1.0F, 1.0F - 4.0F * f};
    }
  }
  ...
  public AxisAlignedBB e(World world, int i, int j, int k) {
    ...
    return AxisAlignedBB.b(af[0] + (float) i, af[1] + (float) j, af[2] + (float) k,
        af[3] + (float) i, af[4] + (float) j, af[5] + (float) k);
  }
