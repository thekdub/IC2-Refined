...
public class TileEntityLuminatorOLD extends TileEntity implements IEnergySink, IEnergyConductor {
  ...
  public void poof() {
    ...
    ExplosionIC2 explosionic2 =
        new ExplosionIC2(this.world, null, 0.5D + (double) this.x, 0.5D + (double) this.y, 0.5D + (double) this.z, 0.5F,
            0.85F, 2.0F);
  ...
  public void burnMobs() {
    ...
    boolean flag4 = false;
    boolean flag5 = false;
    if (this.world.getTypeId(i + 1, j, k) == 0 || this.world.getTypeId(i + 1, j, k) == Block.GLASS.id ||
        this.world.getTypeId(i + 1, j, k) == Ic2Items.reinforcedGlass.id) {
      flag = true;
    }
    ...
    if (this.world.getTypeId(i, j + 1, k) == 0 || this.world.getTypeId(i, j + 1, k) == Block.GLASS.id ||
        this.world.getTypeId(i, j + 1, k) == Ic2Items.reinforcedGlass.id) {
      flag2 = true;
    }
    ...
    if (this.world.getTypeId(i, j, k + 1) == 0 || this.world.getTypeId(i, j, k + 1) == Block.GLASS.id ||
        this.world.getTypeId(i, j, k + 1) == Ic2Items.reinforcedGlass.id) {
      flag4 = true;
    }
    ...
    List list =
        this.world.getEntities(null, axisalignedbb.a(i, j, k).grow(3.0D, 3.0D, 3.0D));
    ...
    for (int j2 = 0; j2 < list.size(); ++j2) {
      ...
      if (entity instanceof EntityMonster) {
        ...
        if (d >= (double) i1 && d <= (double) (l + 1) && d1 >= (double) k1 && d1 <= (double) (j1 + 2) &&
            d2 >= (double) i2 && d2 <= (double) (l1 + 1)) {
