...
public abstract class CropCard {
  ...
  private static final HashMap baseseeds = new HashMap();
  ...
  public static final boolean registerCrop(CropCard cropcard, int i) {
    if (i >= 0 && i < cropCardList.length) {
      if (cropCardList[i] == null) {
        ...
        System.out.println("[IndustrialCraft] Cannot add crop:" + cropcard.name() + " on ID #" + i +
            ", slot already occupied by crop:" + cropCardList[i].name());
        return false;
      }
    }
    else {
  ...
  public boolean onEntityCollision(TECrop tecrop, Entity entity) {
    if (entity instanceof EntityLiving) {
      ...
      if (bukkitentity instanceof Player) {
        ...
        BlockBreakEvent breakev =
            new BlockBreakEvent(player.getWorld().getBlockAt(tecrop.x, tecrop.y, tecrop.z), player);
