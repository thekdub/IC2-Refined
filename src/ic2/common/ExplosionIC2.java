...
public class ExplosionIC2 {
  ...
  public ExplosionIC2(World world, Entity entity, double d, double d1, double d2, float f, float f1, float f2,
                      DamageSource damagesource, String s) {
  ...
  public void doExplosion() {
    if (this.power > 0.0F) {
      ...
      this.entitiesInRange = this.worldObj.a(EntityLiving.class, AxisAlignedBB
          .a(this.explosionX - d, this.explosionY - d, this.explosionZ - d, this.explosionX + d, this.explosionY + d,
              this.explosionZ + d));
      ...
      for (int j = 0; j < 2 * i; ++j) {
        for (int k = 0; k < i; ++k) {
          ...
          this.shootRay(this.explosionX, this.explosionY, this.explosionZ, d1, d2, this.power,
              j % 8 == 0 && k % 8 == 0);
        }
      }
      
      org.bukkit.entity.Entity explode =
          this.exploder == null ? (new EntityTNTPrimed(this.worldObj)).getBukkitEntity() :
              this.exploder.getBukkitEntity();
      ...
      if (!event.isCancelled()) {
        ...
        Platform.playExplodeSound(this.worldObj, (int) this.explosionX, (int) this.explosionY, (int) this.explosionZ,
            this.power);
        ...
        while (true) {
                ...
                while (iterator2.hasNext()) {
                  ...
                  for (int l1 = ((DropData) entry2.getValue()).n; l1 > 0; l1 -= i2) {
                    ...
                    EntityItem entityitem = new EntityItem(this.worldObj,
                        (double) ((float) xzposition.x + this.worldObj.random.nextFloat()) * 2.0D,
                        (double) ((DropData) entry2.getValue()).maxY + 0.5D,
                        (double) ((float) xzposition.z + this.worldObj.random.nextFloat()) * 2.0D,
                        new ItemStack(itemwithmeta.itemId, i2, itemwithmeta.metaData));
          ...
          if ((Boolean) entry.getValue()) {
            double d3 = (float) l + this.worldObj.random.nextFloat();
            double d4 = (float) i1 + this.worldObj.random.nextFloat();
            double d5 = (float) j1 + this.worldObj.random.nextFloat();
            ...
            d10 *= this.worldObj.random.nextFloat() * this.worldObj.random.nextFloat() + 0.3F;
            d6 *= d10;
            d7 *= d10;
            d8 *= d10;
            this.worldObj.a("explode", (d3 + this.explosionX) / 2.0D, (d4 + this.explosionY) / 2.0D,
                (d5 + this.explosionZ) / 2.0D, d6, d7, d8);
    ...
    do {
      ...
      if (i > 0) {
        if (net.minecraft.server.Block.byId[i] instanceof ISpecialResistance) {
          ...
          d9 += ((double) ispecialresistance
              .getSpecialExplosionResistance(this.worldObj, (int) d, (int) d1, (int) d2, this.explosionX,
                  this.explosionY, this.explosionZ, this.exploder) + 4.0D) * 0.3D;
        }
        else {
      ...
      if (d9 > 1000.0D && !ExplosionWhitelist.isBlockWhitelisted(net.minecraft.server.Block.byId[i])) {
        ...
        if (i > 0) {
          ...
          if (!this.destroyedBlockPositions.containsKey(chunkposition) ||
              d5 > 8.0D && (Boolean) this.destroyedBlockPositions.get(chunkposition)) {
      ...
      if (flag) {
        ...
        while (iterator.hasNext()) {
          ...
          if ((entityliving.locX - d) * (entityliving.locX - d) + (entityliving.locY - d1) * (entityliving.locY - d1) +
              (entityliving.locZ - d2) * (entityliving.locZ - d2) <= 25.0D) {
            ...
            if (this.damageSource == IC2DamageSource.nuke && entityliving instanceof EntityHuman &&
                ((EntityHuman) entityliving).name.equals(this.igniter) && entityliving.getHealth() <= 0) {
      ...
      if (d9 > 10.0D) {
        for (int j = 0; j < 5; ++j) {
          this.shootRay(d, d1, d2, this.ExplosionRNG.nextDouble() * 2.0D * 3.141592653589793D,
              this.ExplosionRNG.nextDouble() * 3.141592653589793D, d9 * 0.4D, false);
        }
      }
