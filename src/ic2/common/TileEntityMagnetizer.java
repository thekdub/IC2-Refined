...
public class TileEntityMagnetizer extends TileEntityBlock implements IEnergySink {
  ...
  public void q_() {
    ...
    if (this.ticker-- <= 0) {
      ...
      int j;
      int l;
      for (j = this.y - 1; j > 0 && j >= this.y - 20 && this.energy > 0 &&
          this.world.getTypeId(this.x, j, this.z) == Ic2Items.ironFence.id; --j) {
      ...
      for (j = this.y + 1; j < mod_IC2.getWorldHeight(this.world) && j <= this.y + 20 && this.energy > 0 &&
          this.world.getTypeId(this.x, j, this.z) == Ic2Items.ironFence.id; ++j) {
        l = 15 - this.world.getData(this.x, j, this.z);
        if (l > 0) {
          flag = true;
