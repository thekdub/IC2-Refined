...
public class PointExplosion {
	...
	public void doExplosionA(int i, int j, int k, int l, int i1, int j1) {
				...
				for (k2 = this.explosionZ - k; k2 <= this.explosionZ + j1; ++k2) {
					...
					if (i3 > 0) {
						if (Block.byId[i3] instanceof ISpecialResistance) {
							...
							f = ispecialresistance.getSpecialExplosionResistance(this.worldObj, k1, i2, k2, (double) this.explosionX, (double) this.explosionY, (double) this.explosionZ, this.exploder);
						}
						else {
		...
		List list = this.worldObj.getEntities(this.exploder, AxisAlignedBB.b((double) k1, (double) k2, (double) k3, (double) i2, (double) i3, (double) l3));
		Vec3D vec3d = Vec3D.create((double) this.explosionX, (double) this.explosionY, (double) this.explosionZ);

		for (int i4 = 0; i4 < list.size(); ++i4) {
			Entity entity = (Entity) list.get(i4);
			double d = entity.f((double) this.explosionX, (double) this.explosionY, (double) this.explosionZ) / (double) this.explosionSize;
			if (d <= 1.0D) {
				...
				double d4 = (double) MathHelper.sqrt(d1 * d1 + d2 * d2 + d3 * d3);
				d1 /= d4;
				d2 /= d4;
				d3 /= d4;
				double d5 = (double) this.worldObj.a(vec3d, entity.boundingBox);
				double d6 = (1.0D - d) * d5;
				entity.damageEntity(DamageSource.EXPLOSION, (int) (((d6 * d6 + d6) / 2.0D * 8.0D * (double) this.explosionSize + 1.0D) * (double) this.explosionDamage));
				entity.motX += d1 * d6;
				entity.motY += d2 * d6;
				entity.motZ += d3 * d6;
			}
	...
	public void doExplosionB(boolean flag) {
		this.worldObj.makeSound((double) this.explosionX, (double) this.explosionY, (double) this.explosionZ, "random.explode", 4.0F, (1.0F + (this.worldObj.random.nextFloat() - this.worldObj.random.nextFloat()) * 0.2F) * 0.7F);
		org.bukkit.entity.Entity explode = this.exploder == null ? null : this.exploder.getBukkitEntity();
		Location location = new Location(this.worldObj.getWorld(), (double) this.explosionX, (double) this.explosionY, (double) this.explosionZ);
		...
		if (!event.isCancelled()) {
			...
			for (int i = arraylist.size() - 1; i >= 0; --i) {
				...
				if (flag) {
					double d = (double) ((float) j + this.worldObj.random.nextFloat());
					double d1 = (double) ((float) k + this.worldObj.random.nextFloat());
					double d2 = (double) ((float) l + this.worldObj.random.nextFloat());
					...
					d7 *= (double) (this.worldObj.random.nextFloat() * this.worldObj.random.nextFloat() + 0.3F);
					d3 *= d7;
					d4 *= d7;
					d5 *= d7;
					this.worldObj.a("explode", (d + (double) this.explosionX * 1.0D) / 2.0D, (d1 + (double) this.explosionY * 1.0D) / 2.0D, (d2 + (double) this.explosionZ * 1.0D) / 2.0D, d3, d4, d5);
					this.worldObj.a("smoke", d, d1, d2, d3, d4, d5);
				}

				if (i1 > 0) {
					Block.byId[i1].dropNaturally(this.worldObj, j, k, l, this.worldObj.getData(j, k, l), this.explosionDropRate, 0);
