...
public class ItemScanner extends ElectricItem {
  ...
  public ItemStack a(ItemStack itemstack, World world, EntityHuman entityhuman) {
    if ((this.tier != 1 || ElectricItem.use(itemstack, 50, entityhuman)) && (this.tier != 2 || ElectricItem.use(itemstack, 250, entityhuman))) {
      AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/ODScanner.ogg", true, AudioManager.defaultVolume);
      int i;
      if (this.tier == 2) {
        i = valueOfArea(world, Util.roundToNegInf(entityhuman.locX), Util.roundToNegInf(entityhuman.locY), Util.roundToNegInf(entityhuman.locZ), true);
        Platform.messagePlayer(entityhuman, "SCAN RESULT: Ore value in this area is " + i);
      }
      else {
        i = valueOfArea(world, Util.roundToNegInf(entityhuman.locX), Util.roundToNegInf(entityhuman.locY), Util.roundToNegInf(entityhuman.locZ), false);
