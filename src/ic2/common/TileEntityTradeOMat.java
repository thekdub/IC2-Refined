...
public class TileEntityTradeOMat extends TileEntityMachine implements IPersonalBlock, IHasGui, ISidedInventory, INetworkTileEntityEventListener {
	...
	public void q_() {
		super.q_();
		if (this.inventory[0] != null && this.inventory[1] != null && this.inventory[2] != null && StackUtil.isStackEqual(this.inventory[0], this.inventory[2]) && this.inventory[2].count >= this.inventory[0].count && (this.inventory[3] == null || StackUtil.isStackEqual(this.inventory[1], this.inventory[3]) && this.inventory[3].count + this.inventory[1].count <= this.inventory[3].getMaxStackSize())) {
			boolean flag = false;
			...
			for (int j = 0; j < i; ++j) {
				...
				if (tileentity instanceof IInventory && (!(tileentity instanceof TileEntityPersonalChest) || ((TileEntityPersonalChest) tileentity).owner.equals(this.owner))) {
					...
					if (iinventory.getSize() >= 18) {
						...
						j1 = this.inventory[3] != null ? this.inventory[3].getMaxStackSize() - this.inventory[3].count : this.inventory[1].getMaxStackSize();
						int k1 = Math.min(Math.min(Math.min(this.inventory[2].count / this.inventory[0].count, k / this.inventory[0].count), j1 / this.inventory[1].count), l / this.inventory[1].count);
						if (k1 > 0) {
							...
							ItemStack itemstack1 = StackUtil.getFromInventory(iinventory, new ItemStack(this.inventory[1].id, i2, this.inventory[1].getData()));
							if (itemstack1 != null) {
							...
							StackUtil.putInInventory(iinventory, new ItemStack(this.inventory[0].id, l1, this.inventory[0].getData()));
	...
	public ContainerIC2 getGuiContainer(EntityHuman entityhuman) {
		return this.canAccess(entityhuman) ? new ContainerTradeOMatOpen(entityhuman, this) : new ContainerTradeOMatClosed(entityhuman, this);
	}
