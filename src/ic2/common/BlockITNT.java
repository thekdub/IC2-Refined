...
public class BlockITNT extends BlockIC2Explosive implements IRareBlock {
  ...
  public EntityIC2Explosive getExplosionEntity(World world, float f, float f1, float f2) {
    return this.isITNT ? new EntityItnt(world, f, f1, f2) :
        (new EntityNuke(world, f, f1, f2)).setIgniter(
            (String) this.igniterMap.get(new ChunkCoordinates((int) (f - 0.5F), (int) (f1 - 0.5F), (int) (f2 - 0.5F))));
  }
  
  public void postPlace(World world, int i, int j, int k, EntityLiving entityliving) {
    if (!Platform.isRendering() && !this.isITNT && entityliving instanceof EntityHuman) {
      Platform.log(Level.INFO,
          "Player " + ((EntityHuman) entityliving).name + " placed a nuke at " + i + "/" + j + "/" + k);
    }
