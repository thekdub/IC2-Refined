...
public class BlockScaffold extends BlockTex {
  ...
  public AxisAlignedBB e(World world, int i, int j, int k) {
    ...
    return AxisAlignedBB
        .b((float) i + f, (float) j, (float) k + f, (float) i + 1.0F - f,
            (float) j + 1.0F, (float) k + 1.0F - f);
  }
  ...
  public boolean interact(World world, int i, int j, int k, EntityHuman entityhuman) {
    if (entityhuman.isSneaking()) {
      ...
      if (itemstack != null &&
          (this.material != Material.WOOD || itemstack.id == Item.STICK.id && itemstack.count >= 2) &&
          (this.material != Material.ORE || itemstack.id == Ic2Items.ironFence.id)) {
