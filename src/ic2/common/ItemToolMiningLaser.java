...
public class ItemToolMiningLaser extends ItemElectricTool implements INetworkItemEventListener {
	...
	public ItemStack a(ItemStack itemstack, World world, EntityHuman entityhuman) {
		if (!mod_IC2.portEnableCraftingLaser) {
			...
			if (Keyboard.isModeSwitchKeyDown(entityhuman)) {
				...
				String s = (new String[]{"Mining", "Low-Focus", "Long-Range", "Horizontal", "Super-Heat", "Scatter", "Explosive"})[i];
				...
				switch (i) {
					case 0:
						world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 5.0F, Integer.MAX_VALUE, false));
					...
					case 1:
						...
						break;
					case 2:
						world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 20.0F, Integer.MAX_VALUE, false));
						NetworkManager.initiateItemEvent(entityhuman, itemstack, 2, true);
					case 3:
					default:
						break;
					case 4:
						world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 8.0F, Integer.MAX_VALUE, false, true));
						NetworkManager.initiateItemEvent(entityhuman, itemstack, 4, true);
						break;
					case 5:
						for (int k = -2; k <= 2; ++k) {
							for (int l = -2; l <= 2; ++l) {
								world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 12.0F, Integer.MAX_VALUE, false, (double) (entityhuman.yaw + 20.0F * (float) k), (double) (entityhuman.pitch + 20.0F * (float) l)));
							}
						}
						...
						break;
					case 6:
						world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 12.0F, Integer.MAX_VALUE, true));
			...
			if (!Keyboard.isModeSwitchKeyDown(entityhuman) && nbttagcompound.getInt("laserSetting") == 3) {
				if (Math.abs(entityhuman.locY + (double) entityhuman.getHeadHeight() - 0.1D - ((double) j + 0.5D)) < 1.5D) {
					if (ElectricItem.use(itemstack, 3000, entityhuman)) {
						world.addEntity(new EntityMiningLaser(world, entityhuman, Float.POSITIVE_INFINITY, 5.0F, Integer.MAX_VALUE, false, (double) entityhuman.yaw, 0.0D, (double) j + 0.5D));
	...
	public void onNetworkEvent(int i, EntityHuman entityhuman, int j) {
		switch (j) {
			case 0:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaser.ogg", true, AudioManager.defaultVolume);
				break;
			case 1:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaserLowFocus.ogg", true, AudioManager.defaultVolume);
				break;
			case 2:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaserLongRange.ogg", true, AudioManager.defaultVolume);
				break;
			case 3:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaser.ogg", true, AudioManager.defaultVolume);
				break;
			case 4:
				...
				break;
			case 5:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaserScatter.ogg", true, AudioManager.defaultVolume);
				break;
			case 6:
				AudioManager.playOnce(entityhuman, PositionSpec.Hand, "Tools/MiningLaser/MiningLaserExplosive.ogg", true, AudioManager.defaultVolume);
		}
