...
public class TileEntityTeleporter extends TileEntityBlock implements INetworkTileEntityEventListener {
  ...
  public void q_() {
    ...
    if (Platform.isSimulating()) {
      if (this.world.isBlockIndirectlyPowered(this.x, this.y, this.z) && this.targetSet) {
        ...
        if (chunk != null &&
            chunk.getTypeId(this.targetX & 15, this.targetY, this.targetZ & 15) == Ic2Items.teleporter.id &&
            chunk.getData(this.targetX & 15, this.targetY, this.targetZ & 15) == Ic2Items.teleporter.getData()) {
          this.setActive(true);
          List list = this.world.a(Entity.class, AxisAlignedBB
              .a(this.x - 1, this.y, this.z - 1, this.x + 2,
                  this.y + 3, this.z + 2));
          if (!list.isEmpty()) {
            ...
            while (iterator.hasNext()) {
              ...
              if (entity1.vehicle == null) {
                double d1 = ((double) this.x - entity1.locX) * ((double) this.x - entity1.locX) +
                    ((double) (this.y + 1) - entity1.locY) * ((double) (this.y + 1) - entity1.locY) +
                    ((double) this.z - entity1.locZ) * ((double) this.z - entity1.locZ);
                if (d1 < d) {
                  d = d1;
                  entity = entity1;
                }
  ...
  public void teleport(Entity entity) {
    double d = Math.sqrt((this.x - this.targetX) * (this.x - this.targetX) +
        (this.y - this.targetY) * (this.y - this.targetY) + (this.z - this.targetZ) * (this.z - this.targetZ));
    ...
    if (i != 0) {
      ...
      if (j <= this.getAvailableEnergy()) {
        ...
        Platform.teleportTo(entity, (double) this.targetX + 0.5D, (double) this.targetY + 1.5D + entity.W(),
            (double) this.targetZ + 0.5D, entity.yaw, entity.pitch);
        ...
        if (entity instanceof EntityHuman && d >= 1000.0D) {
          ...
          player.teleport(new Location(player.getWorld(), (double) this.targetX + 0.5D, (double) this.targetY + 1.5D,
              (double) this.targetZ + 0.5D, entity.yaw, entity.pitch), TeleportCause.PLUGIN);
  ...
  public void spawnBlueParticles(int i, int j, int k, int l) {
    for (int i1 = 0; i1 < i; ++i1) {
      this.world.a("reddust", (float) j + this.world.random.nextFloat(),
          (float) (k + 1) + this.world.random.nextFloat(),
          (float) l + this.world.random.nextFloat(), -1.0D, 0.0D, 1.0D);
      this.world.a("reddust", (float) j + this.world.random.nextFloat(),
          (float) (k + 2) + this.world.random.nextFloat(),
          (float) l + this.world.random.nextFloat(), -1.0D, 0.0D, 1.0D);
    }
  ...
  public int getWeightOf(Entity entity) {
    ...
    for (Entity entity1 = entity; entity1 != null; entity1 = entity1.passenger) {
      ...
      else if (!(entity1 instanceof EntityAnimal) && !(entity1 instanceof EntityMinecart) &&
          !(entity1 instanceof EntityBoat)) {
    ...
    if (s.equals("active") && this.prevActive != this.getActive()) {
      if (this.audioSource == null) {
        this.audioSource = AudioManager
            .createSource(this, PositionSpec.Center, "Machines/Teleporter/TeleChargedLoop.ogg", true, false,
  ...
  public void onNetworkEvent(int i) {
    switch (i) {
      case 0:
        AudioManager
            .playOnce(this, PositionSpec.Center, "Machines/Teleporter/TeleUse.ogg", true, AudioManager.defaultVolume);
        AudioManager.playOnce(new AudioPosition(this.world, (float) this.targetX + 0.5F, (float) this.targetY + 0.5F,
                (float) this.targetZ + 0.5F), PositionSpec.Center, "Machines/Teleporter/TeleUse.ogg", true,
            AudioManager.defaultVolume);
