...
public class TileEntityPump extends TileEntityElecMachine implements IHasGui, ISidedInventory {
  ...
  public boolean isWaterBelow() {
    return (this.world.getTypeId(this.x, this.y - 1, this.z) == Block.WATER.id ||
        this.world.getTypeId(this.x, this.y - 1, this.z) == Block.STATIONARY_WATER.id) &&
        this.world.getData(this.x, this.y - 1, this.z) == 0;
  }
  
  public boolean isLavaBelow() {
    return (this.world.getTypeId(this.x, this.y - 1, this.z) == Block.LAVA.id ||
        this.world.getTypeId(this.x, this.y - 1, this.z) == Block.STATIONARY_LAVA.id) &&
        this.world.getData(this.x, this.y - 1, this.z) == 0;
  }
  ...
  public boolean putInChestBucket(int i) {
    return this.putInChestBucket(this.x, this.y + 1, this.z, i) ||
        this.putInChestBucket(this.x, this.y - 1, this.z, i) || this.putInChestBucket(this.x + 1, this.y, this.z, i) ||
        this.putInChestBucket(this.x - 1, this.y, this.z, i) || this.putInChestBucket(this.x, this.y, this.z + 1, i) ||
        this.putInChestBucket(this.x, this.y, this.z - 1, i);
  }
  ...
  public void fountain() {
    ...
    for (int j = 1; j < 4; ++j) {
      if (this.world.getTypeId(this.x, this.y + j, this.z) == 0 ||
          this.world.getTypeId(this.x, this.y + j, this.z) == Block.WATER.id) {
        i = j;
      }
    }
  ...
  public boolean canHarvest() {
    if (!this.isPumpReady()) {
      ...
      return this.inventory[0] != null &&
          (this.inventory[0].id == Ic2Items.cell.id || this.inventory[0].id == Item.BUCKET.id) ||
          this.isBucketInChestAvaible();
    }
  }
  
  public boolean isBucketInChestAvaible() {
    return this.isBucketInChestAvaible(this.x, this.y + 1, this.z) ||
        this.isBucketInChestAvaible(this.x, this.y - 1, this.z) ||
        this.isBucketInChestAvaible(this.x + 1, this.y, this.z) ||
        this.isBucketInChestAvaible(this.x - 1, this.y, this.z) ||
        this.isBucketInChestAvaible(this.x, this.y, this.z + 1) ||
        this.isBucketInChestAvaible(this.x, this.y, this.z - 1);
  }
    ...
    if (s.equals("active") && this.prevActive != this.getActive()) {
      if (this.audioSource == null) {
        this.audioSource = AudioManager
            .createSource(this, PositionSpec.Center, "Machines/PumpOp.ogg", true, false, AudioManager.defaultVolume);
      }
