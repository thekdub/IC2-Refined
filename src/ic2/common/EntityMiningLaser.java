...
public class EntityMiningLaser extends Entity implements IThrowableEntity {
  ...
  static {
    unmineableBlocks = new Block[]{Block.BRICK, Block.OBSIDIAN, Block.LAVA, Block.STATIONARY_LAVA, Block.WATER, Block.STATIONARY_WATER, Block.BEDROCK, Block.GLASS};
  }
  ...
  public EntityMiningLaser(World world, EntityLiving entityliving, float f, float f1, int i, boolean flag) {
    this(world, entityliving, f, f1, i, flag, (double) entityliving.yaw, (double) entityliving.pitch);
  }

  public EntityMiningLaser(World world, EntityLiving entityliving, float f, float f1, int i, boolean flag, boolean flag1) {
    this(world, entityliving, f, f1, i, flag, (double) entityliving.yaw, (double) entityliving.pitch);
    this.smelt = flag1;
  }

  public EntityMiningLaser(World world, EntityLiving entityliving, float f, float f1, int i, boolean flag, double d, double d1) {
    this(world, entityliving, f, f1, i, flag, d, d1, entityliving.locY + (double) entityliving.getHeadHeight() - 0.1D);
  }

  public EntityMiningLaser(World world, EntityLiving entityliving, float f, float f1, int i, boolean flag, double d, double d1, double d2) {
    super(world);
    this.range = 0.0F;
    this.power = 0.0F;
    this.blockBreaks = 0;
  ...
  public void setLaserHeading(double d, double d1, double d2, double d3) {
    double d4 = (double) MathHelper.sqrt(d * d + d1 * d1 + d2 * d2);
    this.motX = d / d4 * d3;
    this.motY = d1 / d4 * d3;
    this.motZ = d2 / d4 * d3;
    this.lastYaw = this.yaw = (float) Math.toDegrees(Math.atan2(d, d2));
    this.lastPitch = this.pitch = (float) Math.toDegrees(Math.atan2(d1, (double) MathHelper.sqrt(d * d + d2 * d2)));
  ...
  public void F_() {
    ...
    if (!Platform.isSimulating() || this.range >= 1.0F && this.power > 0.0F && this.blockBreaks > 0) {
        ...
        if (movingobjectposition.entity != null) {
          if (Platform.isSimulating()) {
            ...
            if (k > 0) {
              ...
              EntityDamageByEntityEvent event = new EntityDamageByEntityEvent(this.owner.getBukkitEntity(), entity.getBukkitEntity(), DamageCause.ENTITY_ATTACK, 1);
              ...
              if (movingobjectposition.entity.damageEntity(damagesource, k) && this.owner instanceof EntityHuman && (movingobjectposition.entity instanceof EntityEnderDragon && ((EntityEnderDragon) movingobjectposition.entity).getHealth() <= 0 || movingobjectposition.entity instanceof EntityComplexPart && ((EntityComplexPart) movingobjectposition.entity).owner.getHealth() <= 0)) {
          ...
          if (!this.canMine(j1)) {
            ...
            if (Block.byId[j1] instanceof ISpecialResistance) {
              ...
              f1 = ispecialresistance.getSpecialExplosionResistance(this.world, k, l, i1, this.locX, this.locY, this.locZ, this) + 0.3F;
            }
            else {
              ...
              if (Block.byId[j1].material == Material.TNT) {
                ...
                if (Block.byId[j1].material == Material.WOOD) {
                  ...
                  while (true) {
                    ...
                    if (!flag && itemstack2.id != j1 && itemstack2.id < Block.byId.length && Block.byId[itemstack2.id] != null) {
                      flag = true;
                      flag1 = false;
                      flag2 = false;
                      ...
                      EntityItem entityitem = new EntityItem(this.world, (double) k + d2, (double) l + d3, (double) i1 + d4, itemstack2);
      ...
      this.range = (float) ((double) this.range - Math.sqrt(this.motX * this.motX + this.motY * this.motY + this.motZ * this.motZ));
      if (this.aU()) {
        this.die();
      }
  ...
  public void explode() {
    if (Platform.isSimulating()) {
      ExplosionIC2 explosionic2 = new ExplosionIC2(this.world, this.owner, this.locX, this.locY, this.locZ, 5.0F, 0.85F, 0.55F);
