  ...
  public boolean onTick(EntityHuman entityhuman, ItemStack itemstack) {
    ...
    if (Platform.isRendering() && entityhuman == Platform.getPlayerInstance()) {
      if (lastJetpackUsed != flag1) {
        if (flag1) {
          if (audioSource == null) {
            audioSource = AudioManager.createSource(entityhuman, PositionSpec.Backpack, "Tools/Jetpack/JetpackLoop.ogg", true, false, AudioManager.defaultVolume);
          }
