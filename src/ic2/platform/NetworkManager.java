...
public class NetworkManager implements IPacketHandler {
	...
	public static void initiateGuiDisplay(EntityPlayer entityplayer, IHasGui ihasgui, int i) {
		try {
			...
			else if (entityplayer.inventory.getItemInHand() != null && entityplayer.inventory.getItemInHand().getItem() instanceof IHandHeldInventory) {
				dataoutputstream.writeByte(1);
					...
					try {
						...
						while (true) {
							...
							do {
								...
								i = Math.min(Math.abs(tileentityfield.te.x - (int) entityplayer.locX), Math.abs(tileentityfield.te.z - (int) entityplayer.locZ));
